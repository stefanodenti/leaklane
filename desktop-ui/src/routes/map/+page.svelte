<script lang="ts">
  ...
  async function loadMap() {
    ...
    selected = null;
    try {
      repoMap = await getRepositoryMap(selectedUrl);
  ...
  function commitLabels(hash: string, map: RepositoryMap) {
    const branches = map.branches.filter((branch) => branch.commit === hash).map((branch) => branch.name);
    const tags = map.tags.filter((tag) => tag.commit === hash).map((tag) => tag.name);
...
<section class="page-head">
  ...
  <button class="ghost" type="button" disabled={!selectedUrl || loadingMap} on:click={loadMap}>
    ...
    <div class="map-main">
        ...
        <section class="repo-map-grid">
          <div class="panel graph-panel">
            <div class="section-title">
              ...
              <span>{repoMap.repository.default_branch || 'default branch n/d'}</span>
            </div>

            <div class="graph-scroll" aria-label="Grafo commit repository">
              <svg class="commit-graph" viewBox={`0 0 ${graphWidth(repoMap)} 360`} role="img">
      ...
      {:else}
        <section class="panel map-intro">
          ...
          <button type="button" disabled={!selectedUrl} on:click={loadMap}>Genera mappa repository</button>
        </section>
      {/if}
    </div>
  </section>
